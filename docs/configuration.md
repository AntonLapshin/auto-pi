...
| `maxRetries` | int | `5` | continues of the same pi session (in addition to the first attempt) for a transient persona/LLM failure; after the budget is exhausted the next dispatch starts a new session |
