...
| File               | Purpose |
|--------------------|---------|
| `orchestrator.js`  | The loop core: read config, acquire lock, check active project/stop, scan state, dispatch, build context, run persona, log, sleep, repeat (plan.md §13.1). Also `writeStopFile` / `isStopped` / `acquireLock` / `releaseLock` / `forceRemoveLock` / `checkLock`, plus the instant-stop/restart helpers: `killLoopInstantly` / `stopLoopInstantly` / `removeStopFile` / `startLoopDetached` / `restartLoop` (plus the still-graceful `waitForLoopExit` used by `/loop-switch`), and the `/loop-switch` helpers: `listProjects` / `resolveProject` / `writeActiveProjectRecord` / `switchProject`. |
...
| `persona-runner.js`| Launches a Pi persona session: unique run ID (doubles as the pi session ID), context passed as a file, output captured in the run dir (plan.md §14 / §29.3). M10 adds full run-log records (plan.md §20.1) and token/cost accounting via `skills/logging/core.js`. M13 adds `runPersonaWithRetry` — a retry/backoff wrapper around the LLM invocation so a single transient provider failure (network blip, 5xx, timeout, rate limit, empty output) CONTINUES the same pi session (`--session-id <runId>` + short "continue" message) with exponential backoff + jitter instead of burning a loop cycle; only a new dispatch (new run ID) starts a new session; non-transient failures fail fast. Tuned via `config.pi.maxRetries` (default 5), `retryBaseDelayMs` (5000), `retryMaxDelayMs` (120000). |
...
- `npm run stop` (or `node scripts/stop.js`) — stop the loop instantly (SIGKILL + stop file; preserves the active-project record).
- `npm run restart` (or `node scripts/restart.js`) — restart the loop instantly (SIGKILL the old loop, start a fresh one).
