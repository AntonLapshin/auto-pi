...
Implements the `/doctor` environment prerequisite check (M1).
...
- `index.ts` — registers the `/doctor` slash command, reusing `core.js` and passing
...
Run it interactively as `/doctor`, or from a shell as `npm run doctor` /
