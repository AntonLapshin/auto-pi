 ...
 *
 * Clarification is agentic: the only hardcoded question asked here is the
 * project *name* (used for the repo slug and display name). All other follow-up
 * questions are generated by an agent that evaluates the idea (see
 * `extensions/seed/agentic-clarify.js`). The user can pick "use assumptions" to
 * skip everything and proceed automatically.
 *
 ...
 * explicit project name (the single hardcoded question).
 */

import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";
import { runSeed } from "./core.js";
// The Question[] type used by the agentic clarifier; buildQuestions remains the
// minimal fallback shape, so we reuse it here for the io.askQuestions contract.
