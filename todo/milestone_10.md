...
**Reference:** original build plan (§20, §28 "Milestone 10"; historical — canonical docs: README.md, docs/architecture.md, docs/commands.md)
...
- [x] Implement the logging skill (skills/logging) writing to `{workspace}/.pi/logs/`:
  ...
  - `summary.jsonl`
  - `latest.log`
- [x] Run-log schema (original plan §20.1): runId, startedAt, finishedAt, persona, trigger, projectName, repo, issueNumber, prNumber, status, action, reason, error, tokensInput, tokensOutput, tokensTotal, durationSeconds, gitSha.
...
- [x] Generate `summary.md` (original plan §20.2): last run info, today's run count/success/fail, tokens used, estimated cost, active work (open issues, open PRs, last merged PR, project state).
- [x] Secret redaction: never write tokens/secrets into logs (original plan §20, §7.2). Implement `logging.redactSecrets` and redaction helper.
