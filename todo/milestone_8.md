...
**Reference:** original build plan (§17, §19, §22, §23, §28 "Milestone 8"; historical — canonical docs: README.md, docs/architecture.md, docs/commands.md)
...
- [x] Write `personas/engineer.md` persona prompt (original plan §17, engineering guidelines §25.1).
- [x] Implement Engineer context packer: issue body + comments, relevant files, test command, policy excerpts, PR review comments when addressing review (original plan §21.1).
- [x] Issue selection: pick a `pi:ready` issue not already in progress.
- [x] Branch creation: `task/{issueNum}-{slug}`.
- [x] Implementation rules enforcing original plan §17.3/§19:
...
- [x] PR creation (original plan §17.6):
...
- [x] Scope-too-large behavior (original plan §17.5): stop, don't partial-implement, leave `PI-NOTE persona=PM reason=scope-too-large action=split`, label `pi:needs-pm` + `pi:blocked`.
- [x] Review-comment handling: address review comments and update PR.
- [x] Squash merge (original plan §17.7):
