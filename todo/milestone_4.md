...
- [ ] Generate `.github/workflows/ci.yml` (plan.md §11.1): checkout, setup-node, `npm ci`, lint, test:coverage, build, upload dist artifact.
- [ ] Generate `.github/workflows/deploy-pages.yml` (plan.md §12.3) using the official actions (configure-pages, upload-pages-artifact, deploy-pages) with pages/id-token permissions and concurrency group.
- [ ] Inject Vite `base: "/{repo}/"` into `vite.config.ts` during scaffolding (plan.md §12.2).
- [ ] Inject demo URL `https://{owner}.github.io/{repo}/` into README during scaffolding.
- [ ] Implement Pages failure handling (plan.md §12.4, §29.1):
...
- [ ] Implement a helper to read the latest workflow run status for the deploy workflow.
